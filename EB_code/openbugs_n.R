...
data = read.csv('data/rural_int.csv')
...
y <- data$crashes
x6 <- data$segment_length
x7 <- data$speed
id_reg <- as.numeric(as.factor(data$county))
off <- data$segment_length
...
for(i in 1:N)
    {
      ...
      log(mu[i]) <-  b1_reg[id_reg[i]]*lnF[i] + b2*x2[i] + b3*x6[i] + b4*x7[i] + b2_reg[id_reg[i]]*z1[i] + b3_reg[id_reg[i]]*z3[i] + b5*off[i] 

       eps[i] ~ dgamma(f[i],t[id_reg[i]])
       f[i] <- 1+z[i] 
       z[i] ~ dbern(k[id_reg[i]])

      ## Log-likelihood
      LL[i] <- (loggam(phi+y[i])-loggam(phi)-loggam(y[i]+1))+(phi)*(log(phi)-log(eps[i]*mu[i]+phi))+y[i]*(log(eps[i]*mu[i])-log(eps[i]*mu[i]+phi)) 
}      
...
for(j in 1:8)
   {  
       k[j] ~ dunif(0, 1)
       t[j]<- (1-k[j])/k[j]
       mean.eps[j] <- (t[j]+2)/(t[j]*(t[j]+1))
       log.mean.eps[j] <- log(mean.eps[j])
       b1_reg[j] ~ dnorm(b1_reg_bar, b1_reg_tau)
       b2_reg[j] ~ dnorm(b2_reg_bar, b2_reg_tau)
       b3_reg[j] ~ dnorm(b3_reg_bar, b3_reg_tau)
       
       ## Transformation
       b0_reg[j] <- log.mean.eps[j] - b1_reg[j]*8.979 - b2*69.159 - b3*14.288 - b4*0.291 -  b2_reg[j]*7.572 -  b3_reg[j]*9.667 - b5*(-1.246)
}      

       ## Priors on regions
       b0_reg_bar <- mean(b0_reg[])
       b1_reg_bar ~ dnorm(0, 0.1)
       b1_reg_tau ~ dgamma(0.1, 0.1)
       b1_reg_var <- 1/(b1_reg_tau)
       b1_reg_sd <- sqrt(b1_reg_var) 
       b2_reg_bar ~ dnorm(0, 0.1)
       b2_reg_tau ~ dgamma(0.1, 0.1)
       b2_reg_var <- 1/(b2_reg_tau)
       b2_reg_sd <- sqrt(b2_reg_var)  
       b3_reg_bar ~ dnorm(0, 0.1)
       b3_reg_tau ~ dgamma(0.1, 0.1)
       b3_reg_var <- 1/(b3_reg_tau)
       b3_reg_sd <- sqrt(b3_reg_var) 
