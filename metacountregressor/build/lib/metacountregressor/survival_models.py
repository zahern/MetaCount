...
The canonical code for this module lives in ``metacountregressor/survival_models.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.survival_models`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.survival_models")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("survival_models")
    _FOUND.append(module)
    return module
