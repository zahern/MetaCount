...
The canonical code for this module lives in ``metacountregressor/pavement_clr.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.pavement_clr`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.pavement_clr")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("pavement_clr")
    _FOUND.append(module)
    return module
