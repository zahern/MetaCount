...
The canonical code for this module lives in ``metacountregressor/output_config.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.output_config`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.output_config")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("output_config")
    _FOUND.append(module)
    return module
