...
The canonical code for this module lives in ``metacountregressor/bivariate_copula.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.bivariate_copula`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.bivariate_copula")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("bivariate_copula")
    _FOUND.append(module)
    return module
