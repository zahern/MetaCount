...
The canonical code for this module lives in ``metacountregressor/sample_data.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.sample_data`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.sample_data")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("sample_data")
    _FOUND.append(module)
    return module
