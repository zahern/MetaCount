...
The canonical code for this module lives in ``metacountregressor/help_guide.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.help_guide`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.help_guide")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("help_guide")
    _FOUND.append(module)
    return module
