...
The canonical code for this module lives in ``metacountregressor/family_search.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.family_search`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.family_search")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("family_search")
    _FOUND.append(module)
    return module
