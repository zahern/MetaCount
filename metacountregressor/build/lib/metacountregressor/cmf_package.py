...
The canonical code for this module lives in ``metacountregressor/cmf_package.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.cmf_package`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.cmf_package")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("cmf_package")
    _FOUND.append(module)
    return module
