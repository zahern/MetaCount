...
# ── Compute per-class gamma sizes from class_membership (if available) ──
cm_idx = spec.class_membership_idx
if cm_idx and len(cm_idx) == C - 1 and K_mem > 0:
    per_class_gamma_sizes = [len(idx) + 1 for idx in cm_idx]
else:
    per_class_gamma_sizes = [K_mem + 1] * (C - 1) if C > 1 else []
    if K_mem > 0 and not cm_idx:
        cm_idx = tuple(tuple(range(K_mem)) for _ in range(C - 1))
gamma_total_size = sum(per_class_gamma_sizes)
...
gamma_init = np.zeros(gamma_total_size)
...
if g_offsets and g_sizes:
    for _c in range(C - 1):
        g_off = g_offsets[_c]
        g_sz = g_sizes[_c]
        se_gamma_list.append(se_all[g_off : g_off + g_sz])
else:
    for _c in range(C - 1):
        Kc = K_mem
        g_off = pindex["class_gamma"][0] + _c * (Kc + 1)
        se_gamma_list.append(se_all[g_off : g_off + Kc + 1])
...
# =====================================================================
# 14.  Membership equation WITH STANDARD ERRORS (per-class)
# =====================================================================
print(f"\n{'=' * 72}")
...
if K_mem > 0:
    for _c in range(C - 1):
        class_num = _c + 2
        idx_tup = cm_idx[_c] if cm_idx else tuple(range(K_mem))
        Kc = len(idx_tup)
        if Kc > 0:
        ...
        if col_names:
            print(f"  log[pi_{class_num}(n) / pi_1(n)] = g_0 + " + " + ".join(f"g_{k+1} * {cn}" for k, cn in enumerate(col_names)))
        else:
            print(f"  log[pi_{class_num}(n) / pi_1(n)] = g_0   (no membership covariates)")
        ...
        for k, name in enumerate(col_names_full):
            g_val = gamma_c[k]
            sg_val = se_c[k]
    ...
    for _c in range(C - 1):
        ...
        z_val = g_val / max(sg_val, 1e-12)
        p_val = 2 * (1 - scipy_stats.norm.cdf(abs(z_val)))
        print(f"  Class {class_num} g_0 = {g_val:>10.4f}  "
              f"(SE = {sg_val:>10.4f}, z = {z_val:>9.3f}, p = {p_val:>9.4f})")

# Marginal class shares (from per-class Z matrices)
...
logits_i_marg = np.column_stack(logits_cols)
logits_full_marg = np.concatenate([np.zeros((N, 1)), logits_i_marg], axis=1)
pi_marg = np.exp(logits_full_marg) / np.exp(logits_full_marg).sum(axis=1, keepdims=True)
pi_marg_mean = pi_marg.mean(axis=0)
print(f"\n  Marginal class shares (population-average priors):")
for c in range(C):
    print(f"    pi_{c + 1} = {pi_marg_mean[c]:.6f}  ({pi_marg_mean[c]:.1%})")

# Class probabilities by membership covariates (using per-class Z mats)
if K_mem > 0 and len(spec.membership_names) > 0:
    print(f"\n  Prior class probabilities by membership covariate profile:")
    for var_name in spec.membership_names[:3]:  # show first 3 membership vars
        var_col = list(spec.membership_names).index(var_name)
        var_vals = np.unique(Xmem_np[:, :, var_col].mean(axis=1))
        var_vals_sample = var_vals[::max(1, len(var_vals)//4)][:5]  # at most 5 values
        for val in var_vals_sample:
            z_example = np.zeros((1, K_mem))
            z_example[0, var_col] = val
            logits_cols = []
            for _c in range(C - 1):
                idx_tup = cm_idx[_c] if cm_idx else tuple(range(K_mem))
                if len(idx_tup) > 0:
                    z_sub = z_example[:, list(idx_tup)]
                    Z_c = np.concatenate([np.ones((1, 1)), z_sub], axis=1)
                else:
                    Z_c = np.ones((1, 1))
                logits_cols.append(Z_c @ gamma_list[_c])
            logits_i = np.column_stack(logits_cols)
            logits_full = np.concatenate([np.zeros((1, 1)), logits_i], axis=1)
            pi_ex = np.exp(logits_full) / np.exp(logits_full).sum(axis=1, keepdims=True)[0]
            pi_str = "  ".join(f"pi_{c+1}={pi_ex[c]:.4f}" for c in range(C))
            print(f"    {var_name}={val:.2f}:  {pi_str}")
