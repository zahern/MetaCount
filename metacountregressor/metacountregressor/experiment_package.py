...
The canonical code for this module lives in ``metacountregressor/experiment_package.py``
(the top-level installed package).  This shim keeps the historical
``metacountregressor.metacountregressor.experiment_package`` import path working and
...
def _canonical_module():
    if _FOUND:
        return _FOUND[0]
    try:
        module = _importlib.import_module("metacountregressor.experiment_package")
    except ImportError:
        module = None
    ...
    module = _importlib.import_module("experiment_package")
    _FOUND.append(module)
    return module
