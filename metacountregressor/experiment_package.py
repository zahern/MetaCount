...
try:
    from ._jax_config import configure_jax, device_summary
...
class StructureEvaluatorLC(StructureEvaluator):
    ...
    def __init__(self, *args, max_latent_classes: int = 3, mutual_exclusion=None, **kwargs):
        ...
        # Human-readable reason why the last build_spec() call returned None
    ...
    def build_spec(self, decision) -> Optional[dict]:
                    ...
                    if var in group and int(roles[i]) != 0:
                        ...
                        if active_in_group > 1:
                            self.last_rejection_reason = (
                                f"mutual exclusion violated: more than one of "
                                f"{list(group)} is active"
                            )
        ...
        for i, var in enumerate(self.vars):
            ...
            if role not in self.allowed_roles.get(var, [0]):
                self.last_rejection_reason = (
                    f"variable '{var}' assigned role {role}, but only "
                    f"{self.allowed_roles.get(var, [0])} allowed"
                )
                return None

            if var in self._unidentifiable and role != 0:
                self.last_rejection_reason = (
                    f"variable '{var}' was flagged unidentifiable "
                    f"({getattr(self, '_unid_reason', {}).get(var, 'collinearity')}) "
                    f"but assigned role {role}"
                )
                return None
        ...
        self.last_rejection_reason = None  # structure decoded successfully
        return spec
    ...
    def fitness(self, decision) -> Union[float, np.ndarray]:
        ...
        try:
            ...
            # ── Multi-class path with warm start ───────────────────
                ...
                result_c = run_with_oom_recovery(
                    polish.run, jnp.array(params_em), label="fitness LC polish"
                )
            ...
            # ── GPU OOM is transient, not structural ─────────────────
            # A structure that failed only because the device ran out of
            # memory must NOT be blacklisted: run_with_oom_recovery has
            # already retried on CPU, so reaching this branch means both
            # attempts failed.  Free caches and reject without banning.
            if _is_oom_error(e):
                try:
                    jax.clear_caches()
                except Exception:
...
class ExperimentBuilder:
    ...
    def _bounded_refit(objective, start_params, bounds, maxiter: int = 500) -> np.ndarray:
        ...
        try:
            ...
            result = solver.run(jnp.array(x0, dtype=jnp.float64), (lb, ub))
    ...
    def run(
        ...
        **algo_kwargs,
        ...
        if algo in ("sa", "hc"):
            ...
            # jobs continue from the incumbent instead of re-exploring at
            # random.  Opt out with warm_start_cache=False.
            warm_start = defaults.pop("warm_start_cache", True)
            if warm_start and "init_solutions" not in defaults:
                D_ = len(evaluator.vars)
                want = 3 * D_ + 2 if has_lc else 2 * D_ + 1
                best_k, best_v = None, None
                for k, v in getattr(evaluator, "cache", {}).items():
                    if isinstance(k, (tuple, list)) and len(k) == want:
                        try:
                            val = float(np.min(v))
                        except Exception:
                            continue
                        if np.isfinite(val) and val < 1e12 and (
                            best_v is None or val < best_v
                        ):
                            best_k, best_v = k, val
                if best_k is not None:
                    defaults["init_solutions"] = [np.asarray(best_k, dtype=float)]
                    print(f"  [resume] warming start from cached best "
                          f"(score={best_v:.4f})")
