...
def get_templates(dest_dir: str = ".") -> None:
    ...
    # templates/ lives next to this file inside the installed package; also
    # probe the nested compat package and a sibling install of the source tree
    # so the lookup works from every layout (wheel, editable, repo checkout).
    here = os.path.dirname(os.path.abspath(__file__))
    candidate_dirs = [
        os.path.join(here, "templates"),
        os.path.join(here, "metacountregressor", "templates"),
    ]
    template_dir = next((d for d in candidate_dirs if os.path.isdir(d)), None)

    if template_dir is None:
        raise FileNotFoundError(
            f"Template directory not found (searched: {candidate_dirs}). "
